...
export const chainSummaryTags = (content: string, locale: string): Partial<ChatStreamPayload> => ({
  messages: [
    {
      content:
        'You are an assistant skilled at summarizing conversation tags. Extract categorization tags from the user\'s input, separated by commas, no more than 5 tags, and translate to the target language. Format requirements:\nInput: {text as JSON-quoted string} [locale]\nOutput: {tags}',
      role: 'system',
    },
    {
