...
export class FileUploadService extends BaseService {
  ...
  private async assertOwnedKnowledgeBase(
    ...
    if (!knowledgeBase) {
      throw this.createNotFoundError('Knowledge base not found or no access permission');
    }
  ...
  async addFilesToKnowledgeBase(
    ...
    try {
      ...
      const failed = uniqueFileIds
        ...
        .map((fileId) => ({ fileId, reason: 'File not found or no access permission' }));
  ...
  async moveFilesBetweenKnowledgeBases(
    ...
    try {
      ...
      this.handleServiceError(error, 'move knowledge base file');
    }
  }
  ...
  async createChunkTask(
    ...
    try {
      ...
      this.handleServiceError(error, 'create chunking task');
    }
  }
  ...
  async getFileChunkStatus(fileId: string) {
    try {
      ...
      this.handleServiceError(error, 'query file chunking status');
    }
  }
